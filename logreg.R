...
# GLM on selected variables from AUC
fit_auc<-glm(gzro~.-sprord-train-lp6_bin-consistent2-cons2b,data=train,family=binomial)
summary(fit_auc)
testpredict<-predict(fit_auc,newdata=test,type="response")
tab=table(test$gzro,testpredict>0.5)
CCR<-sum(diag(tab))/sum(tab)
CCR
